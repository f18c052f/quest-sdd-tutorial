...
**そのテストの自動実行が、本当に動いているとは限らない。** このリポジトリでは実際に
壊れていた。わざと落ちるテストを置いても AI は止まらなかった。この種の仕組みは、
壊れると静かに全部通すだけなので、**壊れていること自体に気づけない。**
だから定期的にわざと落ちるテストを置いて確かめる。やり方は
[docs/setup.md](docs/setup.md#b-5-テストの自動実行を動くようにする) にある。
