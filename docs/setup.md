...
**担当: 済んでいる。** 確認まで終わっている。中身を読んでおくと、あとで壊れたときに直せる。
...
仕組みは [`.claude/hooks/run-editmode-tests.ps1`](../.claude/hooks/run-editmode-tests.ps1)。
AI が「終わりました」と言おうとしたタイミングで自動的に走り、テストが落ちていれば
AI を終わらせない。
...
| いつ | どう走らせるか | 合否の見かた |
| --- | --- | --- |
| Unity を開いている（ふだんはこちら） | 開いている Unity にテストを頼む | 返ってきた結果の「失敗の件数」を見る |
| Unity を閉じている | Unity を裏で起動して走らせる | コマンドの終了コードを見る |

**Unity を開いたままでは `unity test` が使えない。**「既に開かれています」と言われて失敗する。
ふだんは Unity を開いて作業するので、実際にはほぼ上の段が使われる。

#### 確認の結果

わざと落ちるテストを書いて、4つの状態を実際に試した。

| 試した状態 | どうなってほしいか | 結果 |
| --- | --- | --- |
| 落ちるテストが1本ある | 止まる | ✅ 止まった。どのテストがなぜ落ちたかも出た |
| テストが全部通る | 止まらない | ✅ 止まらなかった |
| コンパイルが通っていない | 止まる | ✅ 止まった |
| テストが1本も無い | 止まらないが、黙らない | ✅「この段は今なにも守っていません」と出た |

**この確認で、仕組みが3つ壊れていたことが分かった。**どれも「落ちるテストがあるのに
AI が止まらない」という、一番まずい形の壊れ方だった。書いただけでは動いていなかった。

| 壊れていたところ | なぜ素通りしたか |
| --- | --- |
| PowerShell の設定 | Unity がエラーを出した時点でスクリプトが途中で死に、終了コードが 2 ではなく 1 になっていた。AI が止まるのは 2 のときだけ |
| 文字コード | テスト名が日本語だったため、Unity からの返事が文字化けして読めなくなり、「結果が取れなかった」扱いになっていた |
| コンパイルの確認が無かった | コンパイルが落ちていると、Unity は**前に通ったときのコード**でテストを走らせる。壊れたコードを書いたのに「2件中2件成功」と出た |

3つ目は特に注意。**テストが全部通っていても、コンパイルが落ちていることがある。**
そのときのテスト結果は、いま書いたコードとは無関係。フックはテストの結果とは別に、
コンパイルが通っているかを毎回確かめている。

#### 自分で確かめ直したくなったら

仕組みを疑ったとき、いつでも同じことができる。

1. `Assets/Tests/EditMode/` に、わざと落ちるテストを1本置く（AI に頼んでよい）
2. AI に何か作業をさせて、終わらせようとする
3. **止まれば正常。**止まらなければ、フックが壊れている

Unity を開いた状態で、コマンドからフックだけを動かすこともできる。

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File ".claude/hooks/run-editmode-tests.ps1"
echo $LASTEXITCODE
```

**`2` なら止める判断、`0` なら通す判断。**それ以外の数字が出たら、フック自体が壊れている。

> **テストが1本も無い間、この段は何も守っていない。**いまはその状態。
> D 段階で最初のテストを書くまでは、フックは「1件もありません」と言うだけで通す。
> 止まらなかったからといって、テストが通ったわけではない。
...
だから B-5 で、わざと落ちるテストを書いて本当に止まるかを確かめてある。
**そして実際に壊れていた。**書いただけでは動いておらず、落ちるテストがあっても
AI は止まらなかった。詳細は [B-5](#b-5-テストの自動実行を動くようにする) に書いた。

この手の仕組みは、壊れても何も起きない。静かに全部通すだけなので、
**壊れていることに気づけない。**だから定期的にわざと落ちるテストを置いて、
本当に止まるかを確かめたほうがよい。やり方は B-5 に書いてある。
